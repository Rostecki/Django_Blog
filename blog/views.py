...
def post_detail(request, year, month, day, post):
    post = get_object_or_404(Post, slug=post,
                             ...
                             publish__day=day)
    # Lista aktywych komentarzy dla danego posta
...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        form = EmailPostForm(data=request.POST) # zobaczymy czy działaPi
        if form.is_valid():
            cd = form.cleaned_data
            # wysyłanie maila
            post_url = request.build_absolute_uri(post.get_absolute_url())
            subject = '{} ({}) zachęca do przeczytania {}'.format(cd['name'], cd['email'], post.title)
            message = 'Przeczytaj post "{}" na stronie {}\n\n Komentarz dodanay przez {}: {}'.format(post.title, post_url, cd['name'], cd['comments'])
            send_mail(subject, message, 'admin@mmyblog.com', [cd['to']])
            sent = True
